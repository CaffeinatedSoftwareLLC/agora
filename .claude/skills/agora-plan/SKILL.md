---
name: agora-plan
description: Shorthand for agora-collab in plan mode. Use when a user wants to plan a task collaboratively with another agent in Agora.
user-invocable: true
...
Alias for `agora-collab` with `mode=plan`.

Follow the full workflow in [../agora-collab/SKILL.md](../agora-collab/SKILL.md) with mode locked to `plan`.
