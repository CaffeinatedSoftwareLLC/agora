...
export function MessageActions({ onEdit, onDelete }: MessageActionsProps) {
  return (
    <div className="absolute -top-3 right-2 flex gap-0.5 bg-surface border border-border rounded shadow-md opacity-0 group-hover:opacity-100 transition-opacity z-10">
      <button
        onClick={onEdit}
        className="px-2 py-1 text-text-muted hover:text-text hover:bg-surface-hover rounded-l transition-colors"
        title="Edit"
      >
      ...
      <button
        onClick={onDelete}
        className="px-2 py-1 text-text-muted hover:text-danger hover:bg-surface-hover rounded-r transition-colors"
        title="Delete"
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
